...
# Testzeit!
#
# Suche weiter unten in der Datei die Funktion setup() und rufe von dort
# set_pixel(number, color) für verschiedene Pixel im Ring und deine
# vorher definierten Farben auf.
...
# Die Zählvariable i sagt im wie vielten Durchlauf der Schleife wir uns befinden.
#
# Beispiel:
# for i in range(5):
#     # tut etwas
#
# Führt alles was eingerückt bei "tut etwas" steht genau 5 mal aus.
#
# Schreibe den fehlenden Code, damit alle 12 LEDs ausgeschaltet werden!
def clear_pixels():
    for i in range( ): # 12
        pixel[i] = # (0, 0, 0) als Farbdefinition
...
# Aufgabe 3:
#
# Mit dem was wir über den LED Ring gelernt haben lassen sich nun praktische Dinge bauen - zum Beispiel eine Uhr.
#
# Der Mikrocontroller ESP32 hat sowohl WLAN als auch eine sogenannte Echtzeituhr.
# Über WLAN kann der ESP32 initial einmal bei einer sehr genauen Zeitquelle im Internet die exakte Uhrzeit (und das Datum) erfragen. 
# Durch die Echtzeituhr ist er in der Lage die Zeit genau weiter zu zählen, selbst wenn für einige Zeit nicht die Zeitquelle nicht verfügbar sein sollte.
# Perfekt für unsere eigene Uhr!
#
# Um die exakte Zeit zu erfragen und der Echtzeituhr mitzuteilen musst du nichts weiter tun - Das wird schon in der setup-Funktion 
...
#
# in die Loop-Funktion, um die aktuelle Uhrzeit in jedem Schleifendurchlauf in die Variable current_time (du kannst sie auch anders nennen wenn das einfacher für dich ist) zu speichern.
...
# Aufgabe 4:
#
# Nun wollen wir die Zeit auf dem Pixelring darstellen - überlege welche Möglichkeiten es mit den 12 Pixeln dafür gibt.
# Wenn deine Idee von der Lösung hier abweicht kannst du später noch eine eigene Variante bauen oder diese hier verfeinern!
...
# Das geht wie zuvor, nur dass du als Index welches Pixel du anschalten willst jetzt einfach die Variable hour nutzen kannst.
#
# Such dir eine Farbe aus und schreibe die Logik für die Stunde als Programmcode in die Funktion set_time(hour, minute, second)!
#
# 
# Testzeit!
#
...
# Wir hatten die aktuelle Uhrzeit in der Variablen current_time gespeichert und du hast dir überlegt welche der ausgegebenen Zahlen was bedeutet.
# Aktuell interessieren wir uns nur für die Stunde, die Minute und die Sekunde.
# Einen einzelnen Wert aus dem gesamten Gebilde current_time heraus zu bekommen können wir einfach schreiben
#
...
def loop():
    while True:
        ...
        # Testzeit 2:
        # Nutze set_pixel(number, color) und clear_pixels() auf, um die LEDs in verschiedenen
        # Farben leuchten und blinken zu lassen!
        #
        # Achtung:
        ...
        # Nutze sleep(5) um das An- und Ausschalten zu verzögern!
